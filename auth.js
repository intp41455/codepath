...
(function(){
  ...
  function loadSession(){
    ...
    if(!raw)return;
    try{const d=JSON.parse(raw);if(d.at){accessToken=d.at;user=d.email||null}}catch{}
  }
  function storeSession(at,email,remember){
    const raw=JSON.stringify({at,email:email||user});
  ...
  async function signUp(email,pass,remember=true){
    ...
    accessToken=d.access_token;user=email;
    storeSession(accessToken,email,remember);onUserChange();
    return d;
  }
  async function signIn(email,pass,remember=true){
    ...
    accessToken=d.access_token;user=d.user?.email||email;
    storeSession(accessToken,undefined,remember);onUserChange();
    return d;
  }
  ...
  async function pushProgress(state){
    ...
    try{
      await fetch(SUPABASE_URL+'/rest/v1/codepath_progress?on_conflict=owner_email',{
        method:'POST',
        headers:{...AUTH_HEADS,Authorization:'Bearer '+accessToken,'Prefer':'resolution=merge-duplicates,return=minimal'},
        body:JSON.stringify({owner_email:user||'anon',state,updated_at:new Date().toISOString()})
      });
    }catch{}
  }
  async function pullProgress(){
    ...
    try{
      const r=await fetch(SUPABASE_URL+'/rest/v1/codepath_progress?owner_email=eq.'+encodeURIComponent(user)+'&limit=1',
        {headers:{...AUTH_HEADS,Authorization:'Bearer '+accessToken}});
      if(!r.ok)return null;
  ...
  window.CodepathAuth={signUp,signIn,signOut,pushProgress,pullProgress,openDialog,loadSession,isLoggedIn:()=>!!accessToken,getUser:()=>user,getState:()=>({user,accessToken:!!accessToken})};
